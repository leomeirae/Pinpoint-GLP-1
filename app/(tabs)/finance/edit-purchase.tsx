...
const logger = createLogger('EditPurchaseScreen');
...
export default function EditPurchaseScreen() {
  ...
  useEffect(() => {
    if (!purchaseId) {
      Alert.alert('Erro', 'ID da compra não encontrado');
      router.back();
      return;
    }
    ...
    setMedication(purchase.medication);
    setBrand(purchase.brand || '');
    setDosage(purchase.dosage);
    setQuantity(String(purchase.quantity));

    // Split price into reais and cents
    const totalReais = Math.floor(purchase.total_price_cents / 100);
    const totalCents = purchase.total_price_cents % 100;
    setPriceReais(String(totalReais));
    setPriceCents(String(totalCents).padStart(2, '0'));

    setPurchaseDate(new Date(purchase.purchase_date));
    setLocation(purchase.purchase_location || '');
    setNotes(purchase.notes || '');
    setLoading(false);

    logger.info('Purchase data loaded for editing', { purchaseId });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      logger.info('Purchase updated successfully', { purchaseId, medication, dosage, quantity });

      router.back();
    } catch (error) {
      logger.error('Failed to update purchase', error as Error);
      Alert.alert('Erro', 'Não foi possível atualizar a compra. Tente novamente.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Editar Compra</Text>
        ...
        >
          {isSaving ? (
            ...
            <>
              ...
              <Text style={styles.saveButtonText}>Atualizar compra</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
